...
class DoctorService {
  ...
  static async addDoctorSlots(reqUserId, { date, slots }) {
    ...
    if (!existingSlot) {
      ...
      for (const newSlot of slots) {
        ...
        if (newEnd <= newStart) {
          throw new ErrorHandler(`Invalid slot: endTime must be after startTime (${newSlot.startTime} - ${newSlot.endTime})`, 400);
        }
  ...
  static async deleteSingleTimeSlot(reqUserId, { date, slotId }) {
    ...
    if (!date || !slotId) {
      return next(new ErrorHandler('Date and slotId are required', 400));
    }
    ...
    // Filter out the slot with matching ID
